...
template<>
class Suma<std::string> : public Operacion<std::string> {
public:
    std::string ejecutar(const std::string& a, const std::string& b) override {
        return a + b;
    }
};

// Especialización para std::vector<T>
template<typename T>
class Suma<std::vector<T>> : public Operacion<std::vector<T>> {
public:
...
// Especialización para matrices (vector<vector<T>>)
template<typename T>
class Suma<std::vector<std::vector<T>>> : public Operacion<std::vector<std::vector<T>>> {
public:
